...
from core.widgets.services.open_meteo.animation import WeatherAnimationManager
from core.widgets.services.open_meteo.icons import ICON_MAP, get_weather_icon
from core.widgets.services.open_meteo.utils import (
    ColorFetchWidget,
    copy_painter_state,
...
class HourlyData:
    ...
    def chance_of_rain(self) -> int:
        """Compatibility with WeatherAnimationManager.

        Rain codes: 51-57 (drizzle), 61-67 (rain), 80-82 (rain showers).
        """
        if self.weather_code in {51, 53, 55, 56, 57, 61, 63, 65, 66, 67, 80, 81, 82}:
...
class HourlyDataLineWidget(QFrame):
    ...
    Adapted for Open-Meteo data: uses inline SVG icons via weather codes
    and ``precipitation_probability`` for both rain and snow views.
    """
    ...
    def _get_data_range(self) -> tuple[float, float]:
        ...
        if self.data_type == "rain":
            values = [h.chance_of_rain for h in self.hourly_data]
            return (0.0, max(max(values), 1.0)) if values else (0.0, 100.0)
        if self.data_type == "snow":
            values = [h.chance_of_snow for h in self.hourly_data]
            return (0.0, max(max(values), 1.0)) if values else (0.0, 100.0)
        return (0.0, 100.0)
    ...
    def _prepare_cached_data(self, painter: QPainter):
        """Prepare cached pixmaps for background and foreground drawing."""
        ...
        # Update the weather animation manager
        ...
        for i in range(1, len(self.hourly_data) - 1):
            ...
            if self.data_type == "temperature":
                ...
                data_text = f"{self.hourly_data[i].wind} {wind_unit}"
            else:
        ...
        for i in range(1, len(self.hourly_data) - 1):
            ...
            if self.data_type == "temperature":
                ...
                value_text = f"{self.hourly_data[i].temp}{unit}"
            elif self.data_type == "rain":
                value_text = f"{self.hourly_data[i].chance_of_rain}%"
            elif self.data_type == "snow":
                value_text = f"{self.hourly_data[i].chance_of_snow}%"
            else:
                value_text = f"{self.hourly_data[i].precipitation_probability}%"
